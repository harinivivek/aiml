...
class Attention(nn.Module):
    ...
    def forward(self, features, hidden):
        ...
        hidden = hidden.repeat(1, features.size(1), 1)
...
class DecoderRNNWithAttention(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size, attention_dim, num_layers=1, drop_prob=0.3):
        super(DecoderRNNWithAttention, self).__init__()
        self.embedding = nn.Embedding(vocab_size, embed_size)
        self.lstm = nn.LSTM(embed_size + embed_size, hidden_size, num_layers=num_layers, batch_first=True)
        self.attention = Attention(embed_size, hidden_size, attention_dim)
        self.fcn = nn.Linear(hidden_size, vocab_size)
        self.drop = nn.Dropout(drop_prob)

    def forward(self, features, captions):
        captions = captions[:, :-1]  # Ensure captions have the same batch size as features
        embeds = self.embedding(captions)
        hiddens, _ = self.lstm(torch.cat((features.unsqueeze(1).repeat(1, embeds.size(1), 1), embeds), dim=2))
        context, _ = self.attention(features.unsqueeze(1).repeat(1, embeds.size(1), 1), hiddens)
        outputs = self.fcn(self.drop(context + hiddens))
        return outputs
